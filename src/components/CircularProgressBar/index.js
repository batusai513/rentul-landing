...
class CircularProgressbar extends React.Component {
  constructor(props) {
    ...
    if (props.stackPercentages && Array.isArray(props.stackPercentages)) {
      props.stackPercentages.forEach((p, idx) => {
        state[`path-${idx}`] = props.initialAnimation ? 0 : p.value;
      });
    }
    ...
    if (this.props.initialAnimation) {
      this.initialTimeout = setTimeout(() => {
        this.requestAnimationFrame = window.requestAnimationFrame(() => {
          ...
          if (
            this.props.stackPercentages &&
            Array.isArray(this.props.stackPercentages)
          ) {
            this.props.stackPercentages.forEach(
  ...
  getBackgroundPadding() {
    if (this.props.background) {
      ...
      if (this.props.backgroundPadding == null) {
        return this.props.strokeWidth + TRAIL_OFFSET;
      }
  ...
  getPathRadius() {
    ...
    return (
      FULL_RADIUS -
      (this.props.strokeWidth / 2 + TRAIL_OFFSET) -
    ...
    const stackPercentagesPaths = stackPercentages
      ? stackPercentages.map((p, idx) => {
          ...
          return (
            <path
              key={`path-${idx}`}
              className={`${classes.path} ${(classes.stackPaths &&
                classes.stackPaths[idx]) ||
                STACK_PREFIX + `-${idx}`}`}
              d={pathDescription}
              ...
              style={Object.assign(
                {},
                this.getProgressStyle(this.state[`path-${idx}`], stackOffset),
                { stroke: p.color }
              )}
            />
          );
